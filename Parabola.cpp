...
void Parabola::init(double x1, double y1, double x2, double y2, double length) {
    ...
    if (findDistance(Coordinate(x1, y1), Coordinate(x2, y2)) > (length - 0.0001)) {
        Logger::writeLog(WARNING, "Rope::init(): distance between coordinates is greater than length of parabola\n");
...
Coordinate Parabola::getCoor(double x) const {
    return Coordinate(x, (a * pow(x, 2)) + (b * x) + c);
}
...
bool Parabola::intersects(Line line, Coordinate * p_intersection) const {
    ...
    Coordinate coor1 = getCoor(solveQuadraticPlus(a, newB, newC)), coor2 = getCoor(solveQuadraticMinus(a, newB, newC));
