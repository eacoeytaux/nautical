...
namespace nautical {
    ...
    inline Coordinate operator+(const Coordinate & coor, const physics::Vector & vec) {
        return Coordinate(coor.getX() + vec.getDx(), coor.getY() + vec.getDy());
    }
    
    inline Coordinate & operator+=(Coordinate & coor, const physics::Vector & vec) {
    ...
    inline Line operator+(const Line & line, const physics::Vector & vec) {
    ...
    inline bool physics::Vector::operator==(const Vector & vec) const {
